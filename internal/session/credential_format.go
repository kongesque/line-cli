...
// These wire identifiers and layouts inform this independent credential parser.
...
// inspectCredential returns UNVERIFIED header metadata. Only successful systemd
// decryption of these exact bytes verifies it; enrollment also compares the key.
// Limits deliberately cover small wrapping credentials, not every systemd format.
func inspectCredential(encoded []byte) (credentialProtection, error) {
	var result credentialProtection
	...
	default:
		...
		return credentialProtection{}, errCredential
	}
